...
<h1 class="page-title">
    <i class="voyager-helm"></i> Courses & Intake &nbsp;
</h1>
<a href="" class="btn btn-primary" data-id="{{ $dataTypeContent->getKey() }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{ route('admin.course-intake.delete') }}" id="delete_form" method="POST">
                        {{ method_field('DELETE') }}
                        {{ csrf_field() }}
                        <input type="hidden" name="id" id="courseintakeId">
                                        ...
                                        <thead>
                                            ...
                                            " style="width: 13px;">
                                                ...
                                                <th class="actions text-right dt-not-orderable sorting_disabled"
                                                    ...
                                                    style="width: 167.391px;">Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            ...
                                            <tr role="row" class="odd">
                                                <td>
                                                    <input type="checkbox" name="row_id" id="checkbox_10" value="10">
                                                </td>
                                                <td>
                                                    <div>{{ $course->name }}</div>
                                                </td>
                                                <td>
                                                    <?php
                                                            $intakes = json_decode($course->pivot->intake);
                                                            $length = count($intakes);
                                                            ?>
                                                    @foreach ($intakes as $key => $intake)
                                                    <span>{{ $intake }}</span>
                                                    @if ($key + 1 < $length) , @endif @endforeach </td>
                                                <td>
                                                    2024-04-15 06:30:00
                                                </td>
                                                <td class="no-sort no-click bread-actions">
                                                    <a href="javascript:;" data-id="{{$course->pivot->id}}"
                                                        title="Delete" class="btn btn-sm btn-danger pull-right delete">
                                                        <i class="voyager-trash"></i> <span
                                                            class="hidden-xs hidden-sm">Delete</span>
                                                    </a>
                                                    <a href="{{ route('admin.course-intake.edit', ['id' => $course->pivot->id]) }}"
                                    ...
                                    <div class="dataTables_paginate paging_simple_numbers" id="dataTable_paginate">
                                        <ul class="pagination">
                                            <li class="paginate_button previous disabled" aria-controls="dataTable"
                                                tabindex="0" id="dataTable_previous"><a href="#">Previous</a>
                                            </li>
